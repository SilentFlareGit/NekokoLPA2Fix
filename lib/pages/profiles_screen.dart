  ...
  Future<void> _listProfiles({
    ...
    try {
      ...
      if (isOmapiSessionCorruptedError(e)) {
        _setSwitching(false);
        if (mounted) {
          await _showPremiumConfirmDialog(
            title: AppLocalizations.of(context)!.omapiRebootRequiredTitle,
            message: AppLocalizations.of(context)!.omapiRebootRequiredMessage,
            details: e.toString(),
            confirmLabel: AppLocalizations.of(context)!.ok,
            showCancel: false,
          );
    ...
    try {
      while (retryCount < maxRetries) {
        try {
          ...
          if (canAutomaticallyRetryProfileSwitch(
                e,
      ...
      try {
        ...
        _log.warning('Failed to clear profile switch marker: $markerError');
      }
      if (mounted) {
        setState(() {
          _loading = false;
