...
class ProfileManager {
  ...
  Future<bool> enableProfile(String iccid, {Channel? useChannel}) async {
    // A profile command may have reached the eUICC before radio/SIM refresh breaks OMAPI.
    // It must never be submitted automatically a second time.
    return await _runOnChannel(useChannel, (channel) async {
      ...
      try {
        ...
        if (resp.enableResult != null &&
            ...
            resp.enableResult !=
                EnableProfileResponse_enableResult.profileNotInDisabledState) {
  ...
  Future<bool> disableProfile(String iccid, {Channel? useChannel}) async {
    // See enableProfile: the result is uncertain after a SIM refresh failure.
    return await _runOnChannel(useChannel, (channel) async {
      bool isRefresh = false;
      String iccidHex = iccid;
      if (iccidHex.length % 2 != 0) iccidHex += 'F';
      ...
      try {
        ...
        if (resp.disableResult != null &&
            ...
            resp.disableResult !=
                DisableProfileResponse_disableResult.profileNotInEnabledState) {
